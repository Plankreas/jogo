...
ENEMY = circulo(25,Cor(96, 219, 150))
...
Jogo = definir_estrutura("Jogo", "hero, enemy, plat, game_over", mutavel=True)
...
'''
#EXEMPLOS:
JOGO_INICIAL = Jogo(HERO_INICIAL, ENEMY_INICIAL, PLAT_INICIAL, False)
...
def mover_tudo(jogo):
    ...
    if not (morreu(jogo.hero, jogo.enemy) or caiu(jogo.hero)):
        ...
        if (not colide_plat(jogo.hero, jogo.plat)):
                COLIDE = False
        if colide_plat_enemy(jogo.enemy, jogo.plat):
            COLIDE_ENEMY = True
            jogo.enemy.dy = 0
        if (not colide_plat_enemy(jogo.enemy, jogo.plat)):
            COLIDE_ENEMY = False
